...
bign conv(const string& line) {
    bign ret = 0;
    for (int i = 0; i < line.size(); ++i)
        ret += fib[line.size() - 1 - i] * (line[i] - '0');
    return std::move(ret);
}
...
int main() {
    ...
    bool first = true;
    string linea, lineb;
    while (cin >> linea >> lineb) {
        if (first)
            first = false;
        else
            cout << endl;
        cin.ignore();
        bign ans = conv(linea) + conv(lineb);
        if (ans == 0) {
            cout << "0\n";
            continue;
        }
        while (ans > 0) {
            auto iter = upper_bound(fib, fib + 101, ans);
            for (auto i = iter - 1; i != fib - 1; --i) {
                if (ans >= *i) {
                    ans = ans - *i;
                    cout << "1";
                } else {
