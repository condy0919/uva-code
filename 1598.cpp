...
void trade(int kind) {
    while (!BUY.empty() && !SELL.empty()) {
        if (BUY.rbegin()->first >= SELL.begin()->first) {
            ...
            if (D[aid].size == 0) v1.erase(aid);
            if (D[bid].size == 0) v2.erase(bid);
            if (v1.size() == 0) BUY.erase(D[aid].price);
            if (v2.size() == 0) SELL.erase(D[bid].price);
        } else {
            return;
        }
    }
        ...
        for (int i = 0; i < Q; i++) {
            ...
            if (!strcmp(cmd, "BUY")) {
                ...
                if (D[id].cmd == "BUY") {
                    ...
                    if (BUY[D[id].price].size() == 0) BUY.erase(D[id].price);
                    BUY_VAL[D[id].price] -= D[id].size;
                    D[id].size = 0;
                }
                if (D[id].cmd == "SELL") {
                    SELL[D[id].price].erase(id);
                    if (SELL[D[id].price].size() == 0) SELL.erase(D[id].price);
