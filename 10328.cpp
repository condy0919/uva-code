...
/*
 * http://f0rth3r3c0rd.wordpress.com/2011/09/19/uva-10328-coin-toss/
 * ^ 这种方法更好。
 * 我是看的这个: http://bal4u.dip.jp/cpc/post-43/
 */
...
int main() {
    ...
    for (int i = 2; i <= 100; ++i) {
        ...
        f[i][i - 1] = 2;
        for (int j = 1; j <= i - 2; ++j)
            f[i][j] = f[i - 1][j] * 2 + f[i - 1][j - 1] - f[i - 2][j - 1] * 2 +
                      f[i - j - 1][j - 1] - f[i - j - 2][j];
    }

    int n, k;
    bign ans;
    while (cin >> n >> k)
        cout << accumulate(f[n] + k, f[n] + n + 1, bign(0)) << endl;
