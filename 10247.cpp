...
/*
 * 参考http://blog.csdn.net/metaphysis/article/details/6528414
 * 做完之后才发现原来是高中的题，顺意感觉渣了..
 */
...
int main() {
    ...
    for (int i = 1; i < 22; ++i) {
        for (int j = 1; j < 22; ++j) {
            if (i * j > 21)
                continue;
            f[i][j] = fastpow(f[i][j - 1], i) * factorial(N[i][j - 1] * i) /
                      fastpow(factorial(N[i][j - 1]), i);
        }
    }
